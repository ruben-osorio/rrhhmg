                    ...
                    <div class="row justify-between items-center q-col-gutter-md">
                        <div  class="col col-md-auto " >
                            <div class=" text-h6 text-primary" >
                                Bandeja Eval Curricular
                            </div>
                        </div>
                        <div  class="col-md-auto col-12 " >
                            <template v-if="auth.canView('/bandeja_eval_curricular/add')">
                                <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/bandeja_eval_curricular/add`" class="" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag">
                                <q-breadcrumbs class="q-pa-md">
                                    <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/bandeja_eval_curricular"></q-breadcrumbs-el>
                            ...
                            <div >
                                ...
                                :bordered="false"
                                :columns="app.menus.BandejaEvalCurricularListHeaderItems" 
                                :rows="records"
                                ...
                                <template v-slot:body="props">
                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                                ...
                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                        <q-list dense rounded nav>
                                                            <template v-if="auth.canView('bandeja_eval_curricular/view')">
                                                                <q-item link clickable v-ripple :to="`/bandeja_eval_curricular/view/${props.row.id}`">
                                                                    <q-item-section>
                                                            ...
                                                            <template v-if="auth.canView('bandeja_eval_curricular/edit')">
                                                                <q-item link clickable v-ripple :to="`/bandeja_eval_curricular/edit/${props.row.id}`">
                                                                    <q-item-section>
                                        ...
                                        <q-td  key="id" :props="props">
                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/bandeja_eval_curricular/view/${props.row.id}`">{{ props.row.id }}</q-btn>
                                        </q-td>
                                        <q-td  key="seleccion" :props="props">
                                            {{ props.row.seleccion }}
                                        </q-td>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'bandeja_eval_curricularlist',
		},
		apiPath : {
			type : String,
			default : 'bandeja_eval_curricular/index',
		},
		paginate: {
			type: Boolean,
			default: true,
	...
	const pageTitle = computed({
		get: function () {
			return "Bandeja Eval Curricular"
		}
	});
