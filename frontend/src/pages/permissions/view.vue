                            ...
                            <div >
                                        ...
                                        <q-card  class="q-pa-md nice-shadow-16">
                                            <div class="row q-col-gutter-x-md items-center">
                                                <div class="col">
                                                    ...
                                                    <div class="text-bold">
                                                        <q-btn v-if="item.role_id" @click="app.openPageDialog({ page: 'roles/view', url: `/roles/view/${item.role_id}` , closeBtn: true })" padding="xs" flat color="primary" no-caps >
                                ...
                                <div class="row q-col-gutter-xs justify-start q-ma-md">
                                    <template v-if="auth.canView('permissions/edit')">
                                        <div><q-btn icon="edit" label="Edit" glossy  flat :rounded="false"  no-caps  unelevated   padding="xs" color="positive" title="Editar"  @click="app.openPageDialog({ page:'permissions/edit', url: `/permissions/edit/${item.permission_id}` , closeBtn: true })">
                                        </q-btn></div>
                                    </template>
                                    <template v-if="auth.canView('permissions/delete')">
                                        <div><q-btn icon="delete_sweep" label="Delete" glossy  flat :rounded="false"  no-caps  unelevated   padding="xs" color="negative" title="Borrar"  @click="deleteItem(item.permission_id)">
                                        </q-btn></div>
                                    </template>
                                </div>
                            </div>
