                ...
                <div class="container-fluid">
                    <div class="row justify-between items-center q-col-gutter-md">
                        ...
                        <div  class="col-md-auto col-12 " >
                            <template v-if="auth.canView('/detescala/add')">
                                <q-btn       :rounded="false"  size=""  color="primary" no-caps  unelevated   :to="`/detescala/add`" class="" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag">
                                <q-breadcrumbs class="q-pa-md">
                                    <q-breadcrumbs-el icon="arrow_back" class="text-capitalize" :label="$route.query.tag" to="/detescala"></q-breadcrumbs-el>
                            ...
                            <div >
                                ...
                                :bordered="false"
                                :columns="app.menus.DetescalaListHeaderItems" 
                                :rows="records"
                                ...
                                <template v-slot:body="props">
                                    <q-tr :class="{selected: isCurrentRecord(props.row)}" :props="props">
                                        ...
                                        <q-td  key="iddetesc" :props="props">
                                            <q-btn padding="xs"   :rounded="false"  color="primary"  no-caps  unelevated   flat :to="`/detescala/view/${props.row.iddetesc}`">{{ props.row.iddetesc }}</q-btn>
                                        </q-td>
                                        <q-td  key="coditem" :props="props">
                                            <q-btn v-if="props.row.coditem" @click="app.openPageDialog({ page: 'detitem/view', url: `/detitem/view/${props.row.coditem}` , closeBtn: true })" padding="xs" flat color="primary" no-caps >
                                                <q-icon name="visibility" size="xs"  class="q-mr-xs"></q-icon>  Detitem
                                            </q-btn>
                                        </q-td>
                                                ...
                                                <q-btn icon="menu" padding="xs" round flat color="grey">
                                                    <q-menu auto-close transition-show="flip-right"  transition-hide="flip-left" self="center middle" anchor="center middle">
                                                        <q-list dense rounded nav>
                                                            <template v-if="auth.canView('detescala/view')">
                                                                <q-item link clickable v-ripple :to="`/detescala/view/${props.row.iddetesc}`">
                                                                    <q-item-section>
                                                            ...
                                                            <template v-if="auth.canView('detescala/edit')">
                                                                <q-item link clickable v-ripple :to="`/detescala/edit/${props.row.iddetesc}`">
                                                                    <q-item-section>
                                                            ...
                                                            <template v-if="auth.canView('detescala/delete')">
                                                                <q-item link clickable v-ripple @click="deleteItem(props.row.iddetesc)">
                                                                    <q-item-section>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'detescalalist',
		},
		apiPath : {
			type : String,
			default : 'detescala/index',
		},
		paginate: {
			type: Boolean,
			default: true,
